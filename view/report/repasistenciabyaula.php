...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        // Título
        $this->Cell(0, 10, utf8_decode('REPORTE DE ASISTENCIAS POR AULA'), 0, 1, 'C');
        // Salto de línea
        $this->Ln(20);
...
if ($fechafin != null || $fechafin != '') {
    $sql = "SELECT * FROM alumno INNER JOIN aula ON alumno.Id_Aula = aula.Id_Aula INNER JOIN ingreso ON ingreso.Id_Alumno = alumno.Id_Alumno WHERE aula.Id_Aula = $idaula AND ingreso.Fecha_Ingreso >= '$fechainicio' AND ingreso.Fecha_Ingreso <= '$fechafin'";
} else {
    $sql = "SELECT * FROM alumno INNER JOIN aula ON alumno.Id_Aula = aula.Id_Aula INNER JOIN ingreso ON ingreso.Id_Alumno = alumno.Id_Alumno WHERE aula.Id_Aula = $idaula AND ingreso.Fecha_Ingreso = '$fechainicio'";
}
